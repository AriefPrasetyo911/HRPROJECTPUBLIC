...
export class PerformanceAppraisalCategoryController {
  ...
  @Get(':id')
  async findOne(@Param('id') id: string): Promise<Performance_appraisal_category> {
    return await this.performanceAppraisalCategoryService.findOne(+id);
  }

  @Patch(':id')
  async update(@Param('id') id: string, @Body() data: Prisma.Performance_appraisal_categoryUpdateInput): Promise<Performance_appraisal_category> {
  ...
  @Delete(':id')
  async remove(@Param('id') id: number): Promise<Performance_appraisal_category> {
